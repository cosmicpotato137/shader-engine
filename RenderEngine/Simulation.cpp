...
//    simulation.GetShader()->SetWorkGroups(std::ceil(agents->GetSize() * sizeof(Agent) / 8), 1, 1);
//}

// Function to find k-nearest neighbors for a given agent in a list of agents
std::vector<Agent> FindKNearestNeighbors(const Agent& targetAgent, const std::vector<Agent>& agentList, int k) {
    ...
    for (const auto& agent : agentList) {
        float distance = glm::distance(targetAgent.GetPosition(), agent.GetPosition());
        distancesAndAgents.emplace_back(distance, agent);
    }

    // Sort the vector based on distances
    std::sort(distancesAndAgents.begin(), distancesAndAgents.end(),
...
void Simulation::ImGuiRender()
{
    simulation.ImGuiRender();
}
